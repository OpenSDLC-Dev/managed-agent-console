...
test("session metadata patches, archive refusal and deletion", async ({
  request,
}) => {
  ...
  try {
    ...
    const updated = SessionSchema.parse(
      await ok(
        await request.post(path, {
          data: { title: "Renamed", metadata: { owner: "ops", ticket: "42" } },
        }),
      ),
    );
    ...
    const patched = await ok(
      await request.post(path, { data: { metadata: { ticket: null } } }),
    );
    expect(patched.metadata).toEqual({ owner: "ops" });
    const unchanged = await ok(
      await request.post(path, { data: { metadata: null } }),
    );
    expect(unchanged.metadata).toEqual({ owner: "ops" });
    const archived = SessionSchema.parse(
      await ok(await request.post(`${path}/archive`, { data: {} })),
    );
    expect(archived.archived_at).not.toBeNull();
    expect(
      (await request.post(path, { data: { title: "Cannot edit" } })).status(),
    ).toBe(400);
