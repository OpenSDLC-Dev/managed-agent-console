...
- **UI work reads the design reference first, and fidelity is verified in Chrome.** The console's visual style must stay consistent with Anthropic's Claude Console (a standing product decision, 2026-08-02). Before building or reshaping UI, extract the current reference's design facts (computed fonts, colors, spacing, layout) from platform.claude.com in Chrome; after building, load the local console in Chrome and compare screenshots against the reference side by side. A UI slice is not done until this Chrome fidelity pass has run; note its outcome in the PR. Diverge only where self-hosting demands it, and say so.
