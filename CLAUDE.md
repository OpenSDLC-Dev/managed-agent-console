...
- **UI work reads the design reference first.** Before building or reshaping UI, consult the Claude Console screenshots/notes in the active plan and keep visual consistency with what already shipped; when in doubt, match the reference's information architecture and diverge deliberately, not accidentally.
