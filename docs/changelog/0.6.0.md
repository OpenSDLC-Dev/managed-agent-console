...
The release where the console stops holding one root credential on everyone's behalf. 0.5.0 took its
login _out_ — a Workspace account at the load balancer, IAP in front, no authentication code left in
the pod at all. 0.6.0 puts a sign-in back in, and the whole point is that it authorizes nothing: the
operator signs in against the deployment's own identity provider, and the BFF forwards **the
operator's own token** upstream in place of the management key. The console maps no claims to roles,
keeps no user table, and decides nothing about who may do what. The platform grew OIDC verification
and per-route roles; this release makes the console its relying party and stops there. That reversed
a standing rule rather than extending one — [CLAUDE.md](../../CLAUDE.md)'s fifth principle used to
forbid console SSO and RBAC outright, and now carries the amendment with its date and its reason,
because a principle that changes quietly is worse than one that was never written down.

The sharpest thing the work found is a defect no suite could have reddened. The console must send the
operator's `Authorization: Bearer` **or** the management `x-api-key`, never both: the platform's
dispatcher takes a non-empty `x-api-key` and never looks at the Bearer, so a console attaching both
would have served every operator as root while every test stayed green. Failing closed without a
session is the same idea from the other end — a deployment that opted into SSO must not end up
_less_ protected than one that never did. The rest of the cycle is that first half's mirror image: a
console that had only ever spent credentials learns to mint them — environment keys a self-hosted
worker authenticates on, API keys on the reference's six lifetimes, and the management key the
console itself runs on. Reveal-once, revocable, and expired keys stay listed, because an operator
whose worker stopped connecting needs to see the credential it is failing on. Both halves are
[plan 07](../../docs/plan/07_console-issued-keys.md) and
[plan 08](../../docs/plan/08_console-sso-rbac.md), archived 2026-08-14; what their acceptance runs
proved and broke is in [docs/HISTORY.md](../../docs/HISTORY.md).

The remainder is what measuring turns up where reasoning had not. Three colour-contrast defects had
been shipping unobserved — the focus ring at 1.39:1, the invalid-field indicator at 1.4–1.9:1, and
every destructive control and error message at worst 2.5:1 — and two of them could not have been
fixed by retuning a token at all, because a colour cannot differ enough from a tint of itself. The
fidelity walker turned out to be shooting states it had never reached, or catching one 18% through
its transition. And the documentation lost two-thirds of its bytes to a rule now written into
CLAUDE.md: docs carry what the code cannot.

**Compatibility, measured rather than inherited.** The live tier ran **green as written** on
2026-08-16 — **all seven specs** — against a `deploy/compose` stack built from
[managed-agent-platform](https://github.com/OpenSDLC-Dev/managed-agent-platform)'s `main` at
`v0.2.0-49-gdc04d12`, driving the console at `5d4c75b` — the tree this release tags, under only the
changelog and version-bump commits. Seven rather than 0.5.0's five because this cycle added the
two specs that bear on it most — a console-issued **environment** key authenticating the real worker
lane with revoke closing that lane, and a console-issued **management** key driving the wire API
until Disable stops it. The seventh remains the expensive one and the only one that spends anything:
a real model turn through the human-in-the-loop approve-and-deny path.
