...
0.7.0 brings the platform's expanded operator workflows into the console:
multiagent threads, scheduled Deployments, Memory stores, Dreams and Session
outcomes. Resource inspectors and inline editing keep operators beside their
lists, while Session timelines, approvals and reconnecting live traces make
long-running work easier to inspect.

Validated against the published **managed-agent-platform 0.4.0** Docker images
on 2026-09-17: all 11 model-free live contracts and all 7 browser live tests
passed, including credential issuance/revocation and real-model tool approval
and denial. The live harness was updated for the Dashboard landing page,
Session date presets, expandable tool permissions and inline approval cards.
