...
| Config editor | Rendered form **and** Raw JSON (the wire's shape, round-trip byte-preserving where possible). YAML sugar deferred |
...
- **Raw JSON round-trip fidelity**: the platform stores tool/server/skill entries verbatim; the raw editor must not reorder/normalize on save (edit = replace whole arrays, mirroring update semantics).
